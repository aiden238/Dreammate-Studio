...
| qa-check | 2026-05-26 | 2026-05-29 | 29 | 29 | active | v1.1.0 → v1.2.0 (P3, 카테고리 11). Phase 1:8 + Phase 2:7 + Phase 3:6 + Phase 4:5 + Phase 4.5:1 + Phase 6:2 (Slice 1 entry + Slice 4 final) |
| contract-change | 2026-05-26 | 2026-05-29 | 4 | 4 | active | CC-001 (Option B) + P1~P4 Skill 갱신 + P-X1 phase-start v1.3.0 + **Phase 6 Slice 2 본격 실 변경 ★** (output_schema §9 canonical + §10 Body + agent_io_contract §6 Rewriter v1.1.0 + api_contract §8.3 + ADR-018/019). Phase 3+4+4.5는 contract 변경 0. **Phase 6에서 첫 본격 실 변경 통과 + 회귀 0** |
| meta-retrospective | 2026-05-26 | 2026-05-29 | 6 | 6 | active | Phase 1 + Phase 2 + Phase 3 + Phase 4 + Phase 4.5 + Phase 6 회고 |
| phase-complete | 2026-05-26 | 2026-05-29 | 6 | 6 | active | v1.0.0 → v1.1.0 (P4 §1.5 smoke test) → v1.2.0 (P-X2 §1.6 변경성 시뮬 자동 게이트, Phase 4.5 entry 도입). Phase 1+2+3+4+4.5+6 종료. Phase 4.5 Slice 4 v1.2.0 §1.6 첫 자동 게이트 트리거 (scenario_simulation 5/5) + **Phase 6 Slice 4 두 번째 자동 게이트 (5/5 PASS)** ★ |
| harness-audit | 2026-05-27 | 2026-05-29 | 3 | 3 | active | audit_naming + audit_page_component 모두 자동 호출 (Phase 4 D-1 정규화 보강 + Phase 4.5 Slice 1+4 + Phase 6 Slice 1+4) — 수동 Skill 호출 0 |
| design-review | 2026-05-27 | 2026-05-29 | 5 | 5 | active | Phase 2 Slice 6 (spec-only 첫) + Phase 3 Slice 6 (impl 두 번째 §B) + Phase 4 Slice 4 (impl 세 번째 §B, PlanCard 무수정) + Phase 4.5 Slice 4 (impl 네 번째 §B, PlanCard 9연속) + **Phase 6 Slice 4 (impl 다섯 번째 §B, PlanCard 12연속 무수정)** |
| multi-llm-validation | 2026-05-28 | 2026-05-29 | 3 (1 informal + 2 formal) | 3 | active | Phase 4 informal GPT 검토 + Phase 4.5 entry formal self V1~V4 PASS (외부 placeholder 분리) + **Phase 6 entry formal self ★ 두 번째** V1~V5 PASS (Critic canonical / Rewriter contract / revise_history / fallback / frontend types). P-VALIDATION-FORMAL-001 패턴 두 번째 입증 → 정식 패턴 확정 |
| **agent-io-check** | **2026-05-29** | **2026-05-29** | **1 ★ 첫 정식** | 1 | **active** | **Phase 6 Slice 4 첫 정식 트리거** — Rewriter v1.1.0 contract §6 ↔ rewriter.py 정합 PASS + Critic canonical contract §9.4 ↔ critic.py select_best_plan_index 정합 PASS. type_diff 0 / extra 0 / missing 0 / DeprecationWarning 일치 |
...
| security-review | - | - | 0 | 0 | unused | **Phase 5 entry 첫 트리거 예정** (Auth/RLS 도입) |
