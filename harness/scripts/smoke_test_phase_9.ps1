...
# scripts/smoke_test_phase_9.ps1
# Phase 9 automated smoke test (PowerShell 5.1 compatible)
# Phase 8 baseline (14 checks) + feedback/selection (1 추가) = 15 checks
# Slice 6 — Final QA gate (P-X1 42연속 + scenario_simulation v5 25/25 + 결과저장/피드백 + normalize wiring + Brand Memory 준비)
...
Write-Host '=== Phase 9 Smoke Test (Automated, 15 checks) ===' -ForegroundColor Cyan
Write-Host "Root: $ROOT" -ForegroundColor Gray
Write-Host ''

# --- Step 1/15: pytest backend regression (Phase 8 baseline 249 + Phase 9 신규 ~44 = 293) ---
Write-Host 'Step 1/15: pytest backend (Phase 8 baseline 249 + Phase 9 신규 selection_feedback + plans_feedback_api + critic_canonical_wiring + brand_memory_prep)' -ForegroundColor Cyan
...
# --- Step 2/15: audit_naming (Phase 1 baseline 유지) ---
Write-Host ''
Write-Host 'Step 2/15: audit_naming' -ForegroundColor Cyan
...
# --- Step 3/15: audit_page_component (Phase 5 Slice 3 AuthGuard + login route 신규 — WARN 허용, Phase 9 baseline 계승) ---
Write-Host ''
Write-Host 'Step 3/15: audit_page_component (Phase 5 baseline 2~3 intended WARN, Phase 9 피드백 UI page.tsx inline 계승)' -ForegroundColor Cyan
...
if ($LASTEXITCODE -eq 0) {
    ...
    Record 'audit_page_component' 'WARN' 'drift detected (Phase 5 Slice 3: AuthGuard + /login route intended drift, Phase 9 피드백 UI page.tsx inline 계승)'
}

# --- Step 4/15: next build (Phase 5 Slice 3 /login 추가, Phase 9 피드백 UI page.tsx inline — 11 routes baseline) ---
Write-Host ''
Write-Host 'Step 4/15: next build (11 routes baseline, Phase 9 피드백 UI page.tsx inline wrapper)' -ForegroundColor Cyan
...
# --- Step 5/15: tsc --noEmit ---
Write-Host ''
Write-Host 'Step 5/15: tsc --noEmit' -ForegroundColor Cyan
...
# --- Step 6/15: next lint ---
Write-Host ''
Write-Host 'Step 6/15: next lint' -ForegroundColor Cyan
if (Test-Path $webDir) {
    Push-Location $webDir
...
# --- Step 7/15: .next BUILD_ID 확인 ---
Write-Host ''
Write-Host 'Step 7/15: .next BUILD_ID' -ForegroundColor Cyan
...
# --- Step 8/15: endpoints sanity (Phase 9 신규 select/feedback 포함) ---
Write-Host ''
Write-Host 'Step 8/15: endpoints sanity (Phase 5: plans/start + auth/login + progress / Phase 9: plans/{id}/select + plans/{id}/feedback)' -ForegroundColor Cyan
...
# --- Step 9/15: revise loop integration test (Phase 4.5 baseline 유지) ---
Write-Host ''
Write-Host 'Step 9/15: revise loop integration (test_plans.py revise_history)' -ForegroundColor Cyan
...
# --- Step 10/15: schema_stress_test (Phase 6 P-X2 v2 유지) ---
Write-Host ''
Write-Host 'Step 10/15: schema_stress_test (Phase 6 P-X2 v2)' -ForegroundColor Cyan
...
# --- Step 11/15: DB plans_repo (Phase 5 Slice 2 + Phase 5.5 legacy deprecation) ---
Write-Host ''
Write-Host 'Step 11/15: DB plans_repo (Phase 5 + Phase 5.5 legacy deprecation — test_db.py)' -ForegroundColor Cyan
...
# --- Step 12/15: RLS + SSE (Phase 5 Slice 3+4 baseline) ---
Write-Host ''
Write-Host 'Step 12/15: RLS + SSE (Phase 5 — test_rls + test_sse)' -ForegroundColor Cyan
...
# --- Step 13/15: RAG 5단계 + retrieval (Phase 7 baseline 51 유지) ---
Write-Host ''
Write-Host 'Step 13/15: RAG 5단계 + retrieval (Phase 7 — promotion + quality_filter + eval_rubric + chunking + embedding + retrieval + integration)' -ForegroundColor Cyan
...
# --- Step 14/15: MOA orchestrator + SSE integration + prompt_registry consistency (Phase 8 baseline 26 유지 + normalize wiring) ---
Write-Host ''
Write-Host 'Step 14/15: MOA orchestrator + SSE integration + prompt_registry consistency (Phase 8 — moa_orchestrator + progress_store 브릿지 + Critic v1.1.0 adapter, Phase 9 normalize wiring 정합)' -ForegroundColor Cyan
...
# --- Step 15/15: feedback / selection (Phase 9 신규 — selection_feedback + plans_feedback_api + critic_canonical_wiring + brand_memory_prep) ---
Write-Host ''
Write-Host 'Step 15/15: feedback / selection (Phase 9 — selected_plans + feedback_events graceful + select/feedback API + normalize_to_canonical wiring + Brand Memory 준비 feedback->candidate)' -ForegroundColor Cyan
...
if ($failed -gt 0) {
    Write-Host ''
    Write-Host 'Phase 9 smoke test FAILED' -ForegroundColor Red
    exit 1
}

Write-Host ''
Write-Host 'Phase 9 smoke test PASSED' -ForegroundColor Green
exit 0
