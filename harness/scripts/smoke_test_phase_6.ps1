...
# Phase 6 automated smoke test (PowerShell 5.1 compatible)
# Phase 4.5 baseline (9 checks) + schema_stress_test (10th)
# Slice 4 — Final QA gate (audit_naming + audit_page_component + pytest 144+ + next build 11 routes + tsc + lint + BUILD_ID + endpoints sanity + revise loop + schema stress)
...
Write-Host '=== Phase 6 Smoke Test (Automated, 10 checks) ===' -ForegroundColor Cyan
Write-Host "Root: $ROOT" -ForegroundColor Gray
Write-Host ''

# --- Step 1: pytest backend regression (Phase 4.5 baseline 109 + Phase 6 신규 ~35 = 144+) ---
Write-Host 'Step 1/10: pytest backend (Phase 4.5 baseline + Phase 6 신규 schema/rewriter/critic canonical)' -ForegroundColor Cyan
...
Write-Host ''
Write-Host 'Step 2/10: audit_naming' -ForegroundColor Cyan
...
Write-Host ''
Write-Host 'Step 3/10: audit_page_component' -ForegroundColor Cyan
...
# --- Step 4: next build (Phase 4 11 routes 유지 — Phase 6는 types.ts 수정만, route 추가 0) ---
Write-Host ''
Write-Host 'Step 4/10: next build (11 routes baseline)' -ForegroundColor Cyan
...
# --- Step 5: tsc --noEmit ---
Write-Host ''
Write-Host 'Step 5/10: tsc --noEmit' -ForegroundColor Cyan
...
# --- Step 6: next lint ---
Write-Host ''
Write-Host 'Step 6/10: next lint' -ForegroundColor Cyan
if (Test-Path $webDir) {
    Push-Location $webDir
...
Write-Host ''
Write-Host 'Step 7/10: .next BUILD_ID' -ForegroundColor Cyan
...
Write-Host ''
Write-Host 'Step 8/10: endpoints sanity (FastAPI app routes)' -ForegroundColor Cyan
...
# --- Step 9: revise loop integration test (Phase 4.5 baseline 유지) ---
Write-Host ''
Write-Host 'Step 9/10: revise loop integration (test_plans.py revise_history)' -ForegroundColor Cyan
...
# --- Step 10: schema_stress_test (Phase 6 신규 — P-X2 v2) ---
Write-Host ''
Write-Host 'Step 10/10: schema_stress_test (Phase 6 신규 P-X2 v2)' -ForegroundColor Cyan
...
if ($failed -gt 0) {
    Write-Host ''
    Write-Host 'Phase 6 smoke test FAILED' -ForegroundColor Red
    exit 1
}
