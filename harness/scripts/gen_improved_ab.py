...
def _write_report(
    ...
    if top:
        lines.append(
            ...
            + "이다. 이는 B1 프롬프트가 요구한 구체 후크, 레퍼런스 grounding, 차별화 장치 삽입과 직접 연결된다."
        )
        lines.append("")
    ...
    lines.append("")
    lines.append(
        "판정: B1은 단순히 분량을 늘린 것이 아니라 concept/hook/references/scene 근거에 "
        "구체 앵글과 교체 가능한 예시 anchor를 넣었다. 다만 실제 RAG가 아니라 합성 예시이므로 "
        "데이터 grounding의 품질을 증명한 결과는 아니다."
    )
    lines.append("")
    ...
    lines.append("")
    lines.append(
        "판정: 사람 코멘트의 공통 불만인 genericness는 모든 케이스에서 직접 겨냥했다. "
        "특히 후크/레퍼런스/차별화 장치가 약했던 케이스에서 judge 차원 Δ가 같이 움직였는지가 핵심 근거다."
    )
    lines.append("")
    ...
    lines.append("## 7. 결론")
    lines.append("")
    if aggregate and aggregate["overall_delta_mean"] > 0 and aggregate["regressed_verdict_count"] == 0:
        lines.append(
...
def generate() -> int:
    ...
    for case in cases:
        ...
        for attempt in (1, 2):
            try:
                raw_scores, judge_raw = _judge_one(
