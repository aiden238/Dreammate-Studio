...
**🟢 최신 = Phase 25 위저드(브랜딩 세션) ↔ 4계층 자동 연결 ✅ done (2026-06-04, archive)** — 기능마감(🅑) 2차. 브랜딩 세션 택1(`/branding/select`)에서 topic→**domain**, format→**series** 를 brand 하위에 자동 시드 → 위저드 거치면 /brain 4계층 자동 채움. S1(DomainRepo/SeriesRepo get_or_create 멱등 + plans.py `_auto_create_domain_series` 훅 gated/authed/graceful + 응답 additive). ★ **라이브 데모 PASS**: select{topic,format}→domain "동네 카페 모닝 브이로그"/series "30초 쇼츠" 자동, 멱등(2회=1). ★ 조사 판정: 범용 Discovery/Quick step 위저드는 domain/series discrete 캡처 안 함 → 브랜딩 세션(큐레이션 topic)이 연결점(위저드 재설계는 별건). brand 자동(Phase 17)+Phase 22 repos 재사용. hermetic pytest 735→**749**(+14) + scenario_sim 36/36 + audit 0. CC-032. **branch `phase-25-wizard-4layer-link` (main 머지 진행).** 이월: video 노드 / 개인 PKM 출처 migration / 범용 위저드 연결. **다음 = pending_user_decision.**
...
**🟢 Phase 26 = 4계층 video 노드 + 개인 PKM 출처 (기능마감 완결) — ★ active (entry 2026-06-04)** — 🅑 잔여 2개 완결. ① **video 노드**: VideoProjectRepo 신규(SeriesRepo 패턴) + /me/videos CRUD + 그래프 has_video → User→Brand→Domain→Series→**Video** 전 계층 가시화 ② **개인 PKM 출처**: migration 0007(pkm_entries +source_plan_id) + 추출 훅(plans.py:446, plan_id 가용)이 출처 기록 + 그래프 개인 PKM sourced_from 엣지(Phase 21 브랜드 PKM 로직 재사용). additive/graceful(video 0/출처 없음 → 기존 그래프 byte-identical), migration idempotent. S1(video backend)→S2(pkm source backend)→S3(frontend+라이브+close). entry 5파일 + 4점검(audit 0). baseline pytest 749. branch `phase-26-video-pkm-source`.
