...
**🟢 Phase 27 = 1차 MVP 실사용 마감 — ✅ active (started 2026-06-05)** — ★ 신규 기능 0. GPT 전략 판단(1차 MVP 실사용 마감 → 검증 → 2차 → /GOAL) 객관 검증 = 타당 + 자체 갭분석 합치 → 2차 기능 아닌 **활성화+마감+검증** 우선. 이미 빌드된 1차 MVP(Phase 1~26 + HIP-006~010 머지 a40eb36)를 실사용 가능하게 마감 — 통과 기준 "처음 온 사용자가 도움 없이 director 기획안 1개 생성·저장·brain축적·다음반영". **S0**(HIP→main 머지 ✅ — 홈 진입 카드 B-2 부분 / plan 영속 배선 B-3 / RAG RPC 0008 B-4 토대) → **S1** 실사용 프로파일(B-1, output_mode=director + PKM 주입·추출 + 브랜딩 시드 + plans_repo ON, ★ 코드 default OFF 보존=pytest 802 byte-identical) → **S2** AppShell 네비(B-2 잔여) → **S3** rate_limit 최소(B-7) → **S4** plan 영속 ON 검증 + migration 0001~0008 적용 스크립트(B-3·B-4·M-1) → **S5** 첫-사용자 루프 로컬 e2e + human 채점 핸드오프(B-5). 범위=로컬 마감까지(실 배포·실 migration·human 실채점은 인프라/사용자 액션). baseline pytest **802** + audit_naming 0. branch `phase-27-mvp-realuse-close`. 진입 4점검 = `phases/active/phase-27-mvp-realuse-close/`.
