...
- **잔여**: B(S3 사람 blind — 사용자 채점 대기) / C(위저드 brand 명시 선택 — 다중 brand 시 가치) / 개인 PKM 추출 루프(pkm_entries 채우기, brand 다-S5 대응) / 다-S4 개인+brand 통합 실 e2e(flag ON + Supabase + 시드) / series PKM·orchestrator vector(후속) / 배포 게이트.
