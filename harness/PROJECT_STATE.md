...
**🟢 최신 = Phase 13 출력 확장(Output Enrichment, compact→rich) ✅ done (2026-06-03, archive)** — Phase 12(검증 — 깊이 격차 4.3x 실측) done 후. 핵심 = Phase 12 가 입증한 깊이 격차를 운영 출력에 반영(gated 단계 롤아웃 + additive 스키마, ★ 첫 의도적 출력 변경). **Entry + 6 Slice 전부 ✅ — 깊이 재측정 OFF 0.231 / ON 1.000(≥0.8 PASS) + Critic depth 88점 함정 해소 + frontend rich + ★ 라이브 입증(/generate end-to-end rich HTTP 200). pytest 471→499(기존 471 수정 0 = OFF byte-identical) + 키 0.** ★ rich default 전환 미결정(non_goal — gated OFF 유지). **다음 = Phase 14 pending_user_decision** (rich default 전환 / 위저드 실연결 / 배포 Gate B~G).**
...
**🟢 Phase 15 = director 모드 (output_mode 3rd tier) active (entry 작성 2026-06-03)** — 로드맵 ① director → ② 검증 보강 → ③ PKM/RAG 데이터레이어. project-1 PARKED 제안서(`2026-06-03_commercial-viral-mode-design.md`) 기반. `output_mode` 를 **compact/rich/director** 3-tier 일반화(현 `rich_output_enabled` boolean → enum, backward-compat) + **director**(rich + 연출/리텐션 슬롯 `hook_system`/`retention_architecture`/`scene_breakdown`[director-subset]) gated/additive 추가. ★ director=**LLM-only(데이터레이어 비의존)** → 지금 빌드 가능 + compact/rich byte-identical + commercial_viral(market/audience/brand/conversion)·PKM/RAG=NG(후속). Entry + 6 Slice(S1 enum+스키마 / S2 P-006 director v1.2.0 / S3 wiring / S4 critic retention_design P-007 v1.3.0 / S5 frontend / S6 cost+depth+close). baseline pytest 508 / origin main 748efe2. 폴더 `phases/active/phase-15-director-mode/`. ★ 사용자 "더 깊은 대본기획" 1단계(commercial_viral 디딤돌).
