...
- **★ 정정**: 아래 2026-06-09~10 섹션의 "Phase 27 잔여 = A2 AppShell/A3 rate_limit/A5 e2e"는 **이미 이 트랙(=main)에서 구현·머지됨** (S2 AppShell / S3 rate_limit / S5 첫-사용자 e2e). project-2 핸드오프는 분기 브랜치라 미인지 → 통합으로 충족(재구현 0).
...
- **▶ Phase 27 잔여(A2 AppShell / A3 rate_limit / A5 첫-사용자 e2e)** = ★ **2026-06-10 통합으로 충족됨** — 별도 트랙(이 worktree=main)에서 이미 구현·머지(위 🟢🟢🟢 최신 참조). A1·A4·A7~A11 완료. ⚠️ 이 섹션은 분기 브랜치 시점 기록이라 A2/A3/A5 "미완"은 stale.
...
**🟢 Phase 28 = 2nd brain 학습 루프 — ✅ done (2026-06-08, main af345ae)** — `/GOAL`("쓸수록 내 브랜드를 이해하는 영상기획 2nd brain") 방향. **S1** 모든 대화 저장+학습 연결(홈 legacy `/generate`→plans 흐름 라우팅, 막다른길 제거, plans 0→1·pkm 0→2 검증) → **S2** PKM consolidation(`consolidate_entry` 노이즈필터 + 동일 entry_type 유사도 강화[confidence+0.15]/dedup + `decay_and_prune` ×0.95 감쇠·<0.3 prune, ★ **스키마 변경 0** — 기존 confidence 재활용, 실 Supabase reinforce 0.9→1.0·dedup·noise skip 검증) → **S3** 나만의 컨셉 수렴(`agents/concept_synthesizer` + `GET /me/concept`[gated `concept_surfacing_enabled`] + /brain '내 컨셉' 카드 — 개인 PKM 종합 → 컨셉 한 줄·핵심 기둥 2~4·모순쌍, read-time LLM·영속 0, 라이브 11신호→warm↔neon 모순 자동검출). gated/additive(OFF byte-identical) 회귀 6. 커밋 6bc2a2b~28e2c33. 이월: **컨셉→생성 주입**(현재 표시만).

**🟢 Phase 27 = 1차 MVP 실사용 마감 — ✅ done (2026-06-05, archive)** — ★ 신규 기능 0. GPT 전략 판단(1차 MVP 실사용 마감 → 검증 → 2차 → /GOAL) 객관 검증 = 타당 + 자체 갭분석 합치 → 2차 기능 아닌 **활성화+마감+검증**. 이미 빌드된 1차 MVP(Phase 1~26 + HIP-006~010 머지 a40eb36)를 실사용 가능하게 마감. **S0**(HIP→main 머지) → **S1**(B-1 `APP_PROFILE=realuse` 단일 스위치 → director+PKM 주입·추출+브랜딩 시드+plans_repo ON, ★ default no-op byte-identical, test 6) → **S2**(B-2 AppShell 지속 네비 전 페이지, build 14/14) → **S3**(B-7 rate_limit 최소 generation 신원별 fixed-window→429 gated, test 5) → **S4**(B-3 plan 영속 검증[test 4]+B-4·M-1 migration 적용 스크립트 0001~0008) → **S5**(B-5 첫-사용자 루프 e2e 자동[realuse 부팅 director]+실-런 런북+human 핸드오프). ★ **격리 worktree `phase-27-realuse`**(동시 HIP-006/007 작업자 충돌 회피 — `git add -A` 클로버링 발견 → 사용자 결정). hermetic pytest 802→**813**(기존 802 byte-identical) + scenario_sim 36/36 + audit 0 + frontend build/typecheck/lint. 커밋 S1~S5(363e1f5~9ff6c6d). 회고 `meta/retrospectives/phase-27.md`. ★ 이월: 실 라이브 데모(director 품질·실 영속·PKM 누적, 사용자 opt-in 비용+Supabase) / human 실채점 / AppShell 데스크톱 사이드바 full. **U-5 = phase-27-realuse → main 머지 시 동시 HIP 작업과 충돌 가능(머지 순서 조율).** P-CONCURRENT-ISOLATION-001(신규 후보). ★ **실 라이브 데모 PASS (2026-06-05, 실 LLM+Supabase, 사용자 opt-in)**: migration 0004(pgvector/RAG)+0007+0008 실 적용 → realuse director 기획안 3개 생성(전 슬롯 채움) + plans 영속 rows=1. ★ **라이브가 HIP-008 영속 버그 발견·수정**(`_persist_plan_envelope` 가 plans 에 없는 `envelope` 컬럼 insert→PGRST204→휘발 → 구조화 컬럼 분해 + auth_user_id, 회귀 테스트, pytest 813→**814**). RAG=자체 RAG Lite(SDK 0, OpenAI 임베딩+pgvector RPC). P-LIVE-VERIFY-001 재입증. fix 커밋 981b4aa/eb80e1c. ★ **project-2 트랙(2026-06-09~10) 통합(693957f)**: Gemini RAG 임베딩(임베딩=Gemini/생성=GPT 분리)·agent_io 텔레메트리(intent·rewriter·critic·planning)·측정 A7~A11·BOUNDARIES·platform_evolution·conftest hermeticity.

**🟢 Phase 26 = 4계층 video 노드 + 개인 PKM 출처 (기능마감 완결) — ✅ done (2026-06-04, archive)** — 🅑 잔여 완결. ① **video**: VideoProjectRepo 신규 + /me/videos CRUD(4-hop) + 그래프 has_video → User→Brand→Domain→Series→**Video** 전 계층 ② **개인 PKM 출처**: migration 0007 + 추출 훅 plan_id 기록 + 그래프 개인 PKM sourced_from(`_append_source_provenance` 공유 헬퍼, Phase 21 재사용). S1+S2+S3. ★ **video 라이브 PASS**(CRUD+has_video+미소유 404), 개인 출처=유닛 6. hermetic pytest 749→**779**(+30) + scenario_sim 36/36 + audit 0 + typecheck/lint. CC-033/034. 회고 `meta/retrospectives/phase-26.md`. branch `phase-26-video-pkm-source`.

**🟢 Phase 25 = 위저드(브랜딩 세션) ↔ 4계층 자동 연결 — ✅ done (2026-06-04, archive)** — 기능마감(🅑) 2차. 브랜딩 세션 택1(`/branding/select`)에서 topic→**domain**, format→**series** 자동 시드(brand 하위) → 위저드 거치면 /brain 4계층 자동 채움. **S1**(DomainRepo/SeriesRepo get_or_create 멱등 + plans.py `_auto_create_domain_series` 훅 gated `branding_pkm_seed_enabled`/authed/graceful + 응답 additive domain_id/series_id). ★ **라이브 데모 PASS**: select{topic,format}→domain "동네 카페 모닝 브이로그"/series "30초 쇼츠" 자동 생성 + 멱등(같은 topic 2회=domain 1, 동일 id). ★ 조사 판정: 범용 step 위저드는 domain/series discrete 미캡처 → 브랜딩 세션이 연결점(위저드 재설계는 별건). brand 자동(Phase 17)+Phase 22 repos 재사용. hermetic pytest 735→**749**(+14) + scenario_sim 36/36 + audit 0. CC-032. 이월: video 노드 / 개인 PKM 출처 migration / 범용 위저드 연결. 회고 `meta/retrospectives/phase-25.md`. branch `phase-25-wizard-4layer-link`.

**🟢 Phase 24 = domain/series 편집·삭제 (/brain 4계층 CRUD 완성) — ✅ done (2026-06-04, archive)** — 기능마감(🅑) 1차. Phase 22(생성)+Phase 19(PKM 큐레이션)에 이어 **편집·삭제** → /brain 4계층 CRUD 완성. **S1**(DomainRepo/SeriesRepo update_name+delete + PATCH/DELETE /me/domains·/me/series + 소유검증 _owns_domain/_owns_series 3-hop + domain 삭제 시 series cascade)·**S2**(/brain ✏️rename·🗑delete UI + cascade 경고 + StructureRenameInput). ★ **라이브 데모 PASS**: rename 200(그래프 반영)/미소유 404/DELETE domain→series cascade(summary 0/0) + 브라우저 /brain ✏️🗑 렌더(✏️9/🗑9). additive(그래프 builder 무변경 — read 자동 반영). hermetic pytest 714→**735**(+21) + scenario_sim 36/36 + audit 0 + typecheck/lint. CC-031. 이월(🅑 나머지): 위저드↔4계층 연결 / video 노드 / 개인 PKM 출처 migration. 회고 `meta/retrospectives/phase-24.md`. branch `phase-24-structure-crud`.

**🟢 Phase 23 = 품질 정식화 (실 LLM 전수 eval baseline + human review 정비) — ✅ done (2026-06-04, archive)** — mock eval(구조)만 CI화 → **실 LLM 품질 baseline 확정**. **S1**(golden_set 25 실 LLM rich planning+9차원 critic 전수: overall **4.41**(4.0~4.78)/depth **4.22**(4~5)/18 approve/P0 7/7/광고 1·차단 0 — eval/regression_results/2026-06-04_phase-23-real-baseline.md)·**S2**(Phase 12 S4 kit 2케이스 compact↔rich LLM-judge 대조 + 사용자 채점 시트). ★ **정직한 발견**: ① critic 낙관 편향(88점 함정) → 회귀 기준선이지 절대품질 아님 ② 8차원 overall 은 compact↔rich 무차별(둘 다 ~4.25), **depth(9차원)만 우위**(rich 4~5) → 사람 검증 필요(human 실채점=사용자 deferred) ③ 광고 입력-유래 누수(GS-022). ★ 운영 코드 0 수정(eval=측정만) → pytest **714 불변** + scenario_sim 36/36 + audit 0. 회고 `meta/retrospectives/phase-23.md`. branch `phase-23-quality-baseline`.

**🟢 Phase 22 = domains/series 생성 기능 (4계층 데이터 풍부화) — ✅ done (2026-06-04, archive)** — 4계층(User→Brand→Domain→Series)을 사용자가 직접 생성 → Phase 21 `/brain` 그래프가 빈 4계층에서 실데이터로. **S1**(backend: DomainRepo/SeriesRepo create + POST /me/domains·/me/series + 소유검증 _owns_brand/_owns_domain 2-hop, 401/404/422/503)·**S2**(frontend: /brain "지식 구조 (4계층)" 섹션 — graph 트리 도출 + 생성 입력 + refetch, StructureCreateInput). ★ **라이브 데모 PASS end-to-end**: 생성 API(도메인2/시리즈2, 미소유 404)→/me/pkm-graph 4계층 반영(domains:2/series:2, has_domain/has_series 엣지)→/brain 트리 렌더(브라우저 DOM). additive(기존 /brain·그래프 집계 builder 무변경 — 생성 데이터 자동 반영). hermetic pytest 698→**714**(+16) + scenario_sim 36/36 + audit 0 + typecheck/lint. CC-030(api §8.7 POST). 이월: 편집·삭제 / 위저드 연결 / video 노드 / 데스크톱 그래프 시각. 회고 `meta/retrospectives/phase-22.md`. branch `phase-22-structure-create`.

**🟢 Phase 21 = /brain 4계층 깊이 + 출처 엣지 — ✅ done (2026-06-04, archive)** — Phase 19 `/brain` 그래프 확장: ① **4계층 깊이** domain·series 노드(User→Brand→Domain→Series, DomainRepo/SeriesRepo 신규) + has_domain/has_series 엣지 ② **출처 엣지** 브랜드 PKM(brand_memory.source_plan_id)→`source` 노드+`sourced_from`(dedup). **S1**(backend 집계+repos+graph.py type/kind/summary)·**S2**(frontend PkmGraph domain/series/source 스타일+레이아웃+엣지, types.ts). 전부 **additive/graceful**(domains/series/source 0 = 기존 그래프 byte-identical, summary 0키 additive). hermetic pytest 691→**698**(+7) + scenario_sim 36/36 + audit 0 + typecheck/lint. CC-029(api_contract §8.7). 이월: 개인 PKM 출처(source_plan_id migration) / domains·series 생성 기능 / video 노드 / 프론트 시각 e2e. 회고 `meta/retrospectives/phase-21.md`. branch `phase-21-brain-depth`.

**🟢 Phase 20 = commercial_viral 모드 (output_mode 4th tier) — ✅ done (2026-06-04, archive)** — `output_mode` **4-tier**(compact<rich<director<**commercial_viral**) 완성. commercial_viral = 상업·전략급 기획 브리프(Plan 7슬롯 market_context/audience_psychology/brand_positioning/commercial_conversion/platform_packaging/production_feasibility/measurement_plan + scene 2필드 brand_signal/commercial_signal + Critic **17차원**). **director(Phase 15) 패턴 직접 계승**(effective_output_mode 단일점 + model_dump_for_mode + COMMERCIAL_FIELDS). **S1**(schema 4-tier)·**S2**(P-006 v1.3.0 commercial 프롬프트 10섹션+§3.3)·**S3**(wiring+max_tokens 4500)·**S4**(critic P-007 v1.4.0 17차원)·**S5**(frontend PlanCard commercial 렌더+disclaimer)·**S6**(★라이브 검증+cost §16). ★ **라이브 PASS**(실 LLM: 7슬롯 채움/보장표현 0(보정1)/market·audience "추정:" 표기(보정3)/critic 17/17 approve 4.41/compact 누수 0). 전부 gated/additive/OFF byte-identical. hermetic pytest 668→**691** + scenario_sim 36/36 + audit 0. CC-025~028. ★ 발견·수정: S1 sub-agent nested-exclude 버그 + S3 director parallel_3 절단 잠복버그. 이월: 프론트 시각 e2e / 데이터레이어 enrichment / golden5+human gate. 회고 `meta/retrospectives/phase-20.md`. branch `phase-20-commercial-viral`.
