...
**🟢 Phase 20 = commercial_viral 모드 (output_mode 4th tier) — ★ active (entry 2026-06-04)** — `output_mode` 를 **4-tier**(compact<rich<director<**commercial_viral**)로 확장. commercial_viral = 상업·전략급 기획 브리프(10 additive 슬롯 + CommercialScene 7필드 + Critic 17차원). **director(Phase 15) 패턴 직접 계승**(effective_output_mode 단일점 + model_dump_for_mode + DIRECTOR_FIELDS→COMMERCIAL_FIELDS), 전부 **gated/additive/OFF byte-identical**. ★ 리스크 보정: 보정1(조회수/viral **보장 아님**, 보장표현 차단) · 보정2(**기획 브리프 한정**, 영상제작 영구제외) · 보정3(market/audience 는 데이터레이어 없으면 **LLM 추측 표기**, v1 LLM-only). 선행조건(a rich실사용/b 위저드실연결/c human review)+데이터레이어(PKM/RAG, P16/17) **전부 충족** → validate-before-expand 정합. 슬라이스 S1(schema 4-tier)→S2(P-006 commercial 프롬프트)→S3(wiring)→S4(critic 17차원)→S5(frontend)→S6(라이브 검증+cost+close). entry 6파일 + 4점검(audit 0). 근거 `meta/proposals/2026-06-03_commercial-viral-mode-design.md`. 다음=S1. baseline pytest 668.
