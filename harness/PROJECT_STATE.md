...
- **다음 = S2 측정 보강(체크포인트)**: generic 품질이 아닌 **fit/adherence 측정**(B 출력이 brand_guide/locked_pref/series_format 을 실제로 따르는가) + 사람 blind 준비. ★ 나→가→다 직전 사용자 판단: fit 측정 먼저(권장) vs 가/다 직행.
