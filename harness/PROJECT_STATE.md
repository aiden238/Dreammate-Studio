...
**🟢 최신 = Phase 20 commercial_viral 모드 (output_mode 4th tier) ✅ done (2026-06-04, archive)** — `output_mode` **4-tier 완성**(compact<rich<director<**commercial_viral**) — 깊이 스펙트럼(골격→제작착수→연출→전략) 전부 gated. commercial_viral = 상업·전략급 기획 브리프(Plan 7슬롯 + scene 2필드 + Critic **17차원**). director(P15) 패턴 직접 계승. S1~S6 전부. ★ **라이브 검증 PASS**(실 LLM: 7슬롯 채움 + scene 2필드 + **보장표현 0**(보정1) + market/audience **"추정:" 표기**(보정3) + critic 17/17 approve avg 4.41 + compact 누수 0). hermetic pytest 668→**691** + scenario_sim 36/36 + audit 0. gated/additive(OFF byte-identical). CC-025~028. **branch `phase-20-commercial-viral` (main 머지 진행).** 이월: 프론트 commercial 시각 e2e / 데이터레이어 enrichment(market/audience 실데이터) / golden5+human gate(paid 전). **다음 = (다) /brain 4계층 깊이+출처 엣지 / 배포 Gate B~G — pending_user_decision.**
...
**🟢 Phase 21 = /brain 4계층 깊이 + 출처 엣지 — ★ active (entry 2026-06-04)** — Phase 19 `/brain` 그래프 확장: ① **4계층 깊이** domain·series 노드(User→Brand→Domain→Series 가시화, DomainRepo/SeriesRepo 신규) + has_domain/has_series 엣지 ② **출처 엣지** 브랜드 PKM(brand_memory.source_plan_id)→`source` 노드+`sourced_from` 엣지. 전부 **additive/graceful**(domains/series 0 = 기존 그래프 byte-identical). ★ 한계: 개인 pkm_entries 는 source_plan_id 부재(0006) → 개인 출처 엣지 **이월**(migration). 데이터 모델 충족(domains/series 0001 + source_plan_id 0005). S1(backend 집계+repos+graph.py 스키마)→S2(frontend 렌더+CC+close). entry 5파일 + 4점검(audit 0). baseline pytest 691. branch `phase-21-brain-depth`.
