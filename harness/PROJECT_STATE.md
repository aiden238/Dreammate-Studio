...
**🟢 최신 = Phase 22 domains/series 생성 기능 (4계층 데이터 풍부화) ✅ done (2026-06-04, archive)** — 4계층(User→Brand→Domain→Series)을 사용자가 직접 생성 → Phase 21 `/brain` 그래프가 빈 4계층에서 실데이터로. DomainRepo/SeriesRepo **create** + POST /me/domains·/me/series(소유검증 RLS) + /brain "지식 구조" 추가 UI(StructureCreateInput). S1(backend)+S2(frontend). ★ **라이브 데모 PASS end-to-end**: 생성 API(도메인2/시리즈2, 미소유 404)→/me/pkm-graph 4계층 반영(domains:2/series:2, has_domain/has_series)→/brain 트리 렌더. additive(기존 /brain·그래프 builder 무변경). hermetic pytest 698→**714** + scenario_sim 36/36 + audit 0 + typecheck/lint. CC-030. **branch `phase-22-structure-create` (main 머지 진행).** 이월: domain/series 편집·삭제 / 위저드 연결 / video 노드 / 개인 PKM 출처 migration / 데스크톱 그래프 시각. **다음 = 배포 Gate B~G — pending_user_decision.**
...
**🟢 Phase 23 = 품질 정식화 (실 LLM 전수 eval baseline + human review 정비) — ★ active (entry 2026-06-04)** — 지금까지 mock-deterministic eval(구조)만 CI화 → **실 LLM 품질 baseline** 확정. S1(golden_set 25 실 LLM rich planning+9차원 critic 전수 → overall/depth/verdict 분포 + P0 pass)·S2(Phase 12 S4 human kit LLM-judge 대조 + 사용자 채점 시트, 실채점=사용자 deferred). ★ 운영 코드 0 수정(eval=측정만, behavior-preserving). 비용 ~$0.3~0.5(opt-in 1회). 근거 로드맵 D + Gate D 전제. eval 조사: real 경로 wire됨(graceful)이나 전수 baseline 미확정 + human kit 실채점 대기. entry 5파일 + 4점검(audit 0). baseline pytest 714. branch `phase-23-quality-baseline`.
