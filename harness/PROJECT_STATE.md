...
- ★ **"가"(실연결) 완료** = 신원 + brand_memory 구속 주입(unit+live). 
- **다음 = B(S3 사람 blind 채점 검토 — 사용자 채점 대기) → C(위저드 brand_id 명시 선택) 순차**. 잔여 = 다-S3(`pkm_entries` 개인 PKM·개인별 메모리) + 다-S4(개인+brand 통합 e2e).
