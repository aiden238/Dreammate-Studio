...
**🟢 최신 = Phase 23 품질 정식화 (실 LLM 전수 eval baseline + human review 정비) ✅ done (2026-06-04, archive)** — mock eval(구조)만 CI화된 상태 → **실 LLM 품질 baseline 확정**. S1(golden_set 25 실 LLM rich planning+9차원 critic 전수: overall **4.41**/depth **4.22**/18 approve/P0 7/7/광고 1·차단 0)·S2(Phase 12 S4 kit compact↔rich LLM-judge 대조 + 사용자 채점 시트). ★ **정직한 발견**: critic 낙관 편향(88점 함정) → 회귀 기준선이지 절대품질 아님 + 8차원은 compact↔rich 무차별, **depth(9차원)만 우위** → 사람 검증 필요(human 실채점=사용자 deferred). ★ 운영 코드 0 수정(eval=측정만, behavior-preserving) → pytest **714 불변** + scenario_sim 36/36 + audit 0. **branch `phase-23-quality-baseline` (main 머지 진행).** 이월: human 실채점 회수→human↔LLM diff / 광고 입력 필터 / 3안·compact 전수 baseline. **다음 = 배포 Gate B~G — pending_user_decision.**
...
**🟢 Phase 24 = domain/series 편집·삭제 (/brain 4계층 CRUD 완성) — ★ active (entry 2026-06-04)** — 기능마감(🅑) 1차. Phase 22(domain/series 생성)+Phase 19(PKM 큐레이션)에 이어 **편집·삭제** 추가 → /brain 4계층 구조 CRUD 완성. PATCH/DELETE /me/domains·/me/series(소유검증 RLS, domain 삭제 시 series cascade=Supabase FK ON DELETE CASCADE / in-memory 명시) + /brain 편집/삭제 UI. ★ 라이브 데모. additive(생성/조회/그래프 무변경 — 그래프 read 가 자동 반영). 패턴 재사용(Phase 19 S4 PKM 큐레이션 + Phase 22 create). S1(backend)→S2(frontend+라이브+close). entry 5파일 + 4점검(audit 0). baseline pytest 714. branch `phase-24-structure-crud`. 범위 밖(다음): 위저드↔4계층 연결 / video 노드 / 개인 PKM 출처 migration.
