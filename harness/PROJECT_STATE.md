...
**🟢 최신 = Phase 24 domain/series 편집·삭제 (/brain 4계층 CRUD 완성) ✅ done (2026-06-04, archive)** — 기능마감(🅑) 1차. Phase 22(생성)+Phase 19(PKM 큐레이션)에 이어 domain/series **편집·삭제** → /brain 4계층 구조 **CRUD 완성**. S1(DomainRepo/SeriesRepo update/delete + PATCH/DELETE /me/domains·/me/series + 소유검증 _owns_series 3-hop + domain 삭제 시 series cascade)·S2(/brain ✏️편집·🗑삭제 UI + cascade 경고). ★ **라이브 데모 PASS end-to-end**: rename 200(그래프 반영)/미소유 404/DELETE domain→series cascade(summary 0/0) + 브라우저 /brain ✏️🗑 렌더. additive(그래프 builder 무변경). hermetic pytest 714→**735**(+21) + scenario_sim 36/36 + audit 0 + typecheck/lint. CC-031. **branch `phase-24-structure-crud` (main 머지 진행).** 이월(🅑 나머지): 위저드↔4계층 연결 / video 노드 / 개인 PKM 출처 migration. **다음 = pending_user_decision.**
...
**🟢 Phase 25 = 위저드(브랜딩 세션) ↔ 4계층 자동 연결 — ★ active (entry 2026-06-04)** — 기능마감(🅑) 2차. 브랜딩 세션 택1(`/plans/{id}/branding/select`)에서 선택 topic→**domain**, format→**series** 를 brand 하위에 자동 시드 → 위저드 거치면 /brain 4계층 자동 채움(수동 생성 불필요). 멱등(get_or_create)·gated(`branding_pkm_seed_enabled` 재사용)·graceful. 응답 additive(domain_id/series_id). ★ 조사 판정: 범용 Discovery/Quick step 위저드는 domain/series discrete 캡처 안 함 → 브랜딩 세션(큐레이션 topic)이 자연스러운 연결점(범용 위저드 재설계는 별건). brand 자동 생성(Phase 17) + DomainRepo/SeriesRepo.create(Phase 22) 재사용. S1(backend 훅)→S2(라이브 데모+close). entry 5파일 + 4점검(audit 0). baseline pytest 735. branch `phase-25-wizard-4layer-link`.
