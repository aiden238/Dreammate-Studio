...
- **잔여**: B(S3 사람 blind — 사용자 채점 대기) / C(위저드 brand 명시 선택 — 다중 brand 시) / 다-S4 통합 실 e2e(flag ON + 서버 + Supabase 시드 라이브) / Phase 17 phase-complete(회고/archive) / series PKM·orchestrator vector·commercial_viral(후속) / 배포 게이트.
