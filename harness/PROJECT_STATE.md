...
**🟢 최신 = Phase 21 /brain 4계층 깊이 + 출처 엣지 ✅ done (2026-06-04, archive)** — Phase 19 `/brain` 그래프 확장: ① 4계층 domain·series 노드(User→Brand→Domain→Series, DomainRepo/SeriesRepo 신규) + has_domain/has_series ② 브랜드 PKM 출처(source_plan_id→source 노드+sourced_from, dedup). S1(backend)+S2(frontend+CC-029). ★ additive/graceful — domains/series/source 0 = 기존 그래프 byte-identical(summary 0키만 additive). hermetic pytest 691→**698** + scenario_sim 36/36 + audit 0 + 프론트 typecheck/lint. 한계: 개인 PKM 출처는 source_plan_id 부재로 이월(migration) / domains·series 실데이터 희소(생성 경로 미비). **branch `phase-21-brain-depth` (main 머지 진행).** **다음 = 배포 Gate B~G / domains·series 생성 기능 / 개인 PKM 출처 migration — pending_user_decision.**
...
**🟢 Phase 22 = domains/series 생성 기능 (4계층 데이터 풍부화) — ★ active (entry 2026-06-04)** — 4계층(User→Brand→Domain→Series)을 사용자가 직접 생성 → Phase 21 `/brain` 그래프가 실데이터로 채워짐. 현재 domains/series 는 테이블+read repo(Phase 21)만 있고 **생성 경로 0** → 그래프 항상 빈 4계층. 본 phase: DomainRepo/SeriesRepo **create** + POST /me/domains·/me/series(소유검증 RLS) + /brain 추가 UI + ★ **라이브 데모**(생성→그래프 4계층). additive(기존 /brain 읽기/큐레이션·그래프 집계 무변경 — 생성 데이터 자동 반영). S1(backend create+endpoints+CC)→S2(frontend UI+라이브+close). entry 5파일 + 4점검(audit 0). baseline pytest 698. branch `phase-22-structure-create`.
