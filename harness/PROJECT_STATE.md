...
- **다음 = B(S3 사람 blind — 사용자 채점 대기) / C(위저드 brand 선택 — 이제 brand 존재) / 다-S3(`pkm_entries` 개인 PKM, auth_user_id 키 — brand 무관) / brand_memory 추출 루프 배선**. 사용자 순서 = C 진행 중(brand foundation 완료) → B(채점 시).
