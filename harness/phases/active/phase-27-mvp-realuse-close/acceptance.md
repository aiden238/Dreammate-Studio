...
- [ ] **A1 (S1) 실사용 프로파일**: env 프로파일 1개 정의 + 문서화. 그 프로파일로 서버 기동 시 effective output_mode=director + PKM 주입·추출 ON + 브랜딩 시드 ON + plans_repo ON. **코드 default OFF 유지 → 기존 hermetic pytest 802 byte-identical**. (자동: 프로파일 env로 effective_output_mode/flag 해석 test)
...
| 기준 | 방법 | 자동/수동 |
|---|---|---|
| A1 | 프로파일 env → effective_output_mode/flag 해석 test + OFF 802 불변 | 자동 |
