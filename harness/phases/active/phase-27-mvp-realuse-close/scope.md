...
- **S1 실사용 프로파일 (B-1)**: 핵심 루프를 ON으로 켜는 env 프로파일 정의 + 문서. output_mode=director + PKM 주입·추출(brand/personal) + 브랜딩 시드 + plans_repo. 코드 default는 OFF 유지.
...
```
backend/fastapi/config.py                 # S1 프로파일 헬퍼/문서 (default 불변)
