...
- HIP-008(이미 main 머지, a40eb36)이 토대를 깔아둠: 홈 진입 카드(B-2 부분) · plan 영속 배선(B-3, gated) · match_approved_knowledge RPC(B-4 SQL). → Phase 27 = **신규 빌드가 아니라 활성화 + 마감 + 검증**.
...
- flag 활성 = **운영 env 프로파일**로 ON. 글로벌 코드 default는 OFF 유지(기존 pytest 802 byte-identical).
