...
- 코드 default flag **flip 금지** — 기존 pytest 802 byte-identical 유지. 활성은 **env 프로파일(APP_PROFILE=realuse)**로만.
...
## 작업 분리 (동시 HIP-006/007)

- HIP-006/007(critic·planning 텔레메트리, agent_io_log DB 적재, human N=5, meta/improvement_*) = **다른 작업자 영역, 건드리지 않음**. Phase 27은 격리 worktree에서 진행.
