...
- 실사용 활성 = **env 프로파일**(APP_PROFILE=realuse) 방식. 글로벌 코드 default flag는 OFF 유지(기존 pytest 802 byte-identical).
- output_mode = **director** (사용자 결정 2026-06-05).
- 범위 = **로컬 마감까지** (실 배포·실 migration 실행·human 실채점은 제외, 스크립트/kit 핸드오프까지).
- 작업 = **격리 worktree `phase-27-realuse`**(동시 HIP-006/007 작업자 충돌 회피, 사용자 결정).
...
- **U-5**: 격리 worktree → main 머지 시 동시 HIP-006/007 작업과의 머지 순서/충돌(같은 파일 거의 없으나 config.py/STATE 동시 가능) — phase-complete 머지 시점 재확인.
...
- HIP-006/007 파일(critic.py·agents/planning.py·observability/agent_io_log.py·meta/improvement_*) = forbidden(다른 작업자).
