...
| **27** | **1차 MVP 실사용 마감 (실사용 프로파일 + 네비 + rate_limit + 영속·운영 반영 + 첫-사용자 e2e)** | **done** (2026-06-05) | ★ 신규 기능 0 — 이미 빌드된 1차 MVP(Phase 1~26 + HIP-006~010)를 **실사용 가능하게 마감**. GPT 전략 판단 + 갭분석 합치(코드 高/실사용·운영 中/품질 弱) → 2차 기능 아닌 **활성화+마감+검증** 우선. S0(HIP→main 머지 a40eb36 ✅) + S1 실사용 프로파일(B-1, APP_PROFILE=realuse → output_mode=director + PKM/시드/plans_repo ON, 코드 default OFF 보존) + S2 AppShell 네비(B-2 잔여) + S3 rate_limit 최소(B-7) + S4 plan 영속 ON 검증+migration 적용 스크립트(B-3·B-4·M-1) + S5 첫-사용자 루프 로컬 e2e+human 채점 핸드오프(B-5). 통과 = "처음 온 사용자가 도움 없이 director 기획안 1개 생성·저장·brain축적·다음반영". ★ 격리 worktree `phase-27-realuse`(동시 HIP-006/007 작업자 `git add -A` 충돌 회피). 결과: hermetic pytest 802→**813**(기존 byte-identical) + scenario_sim 36/36 + audit 0 + frontend build 14/14. 자동검증 close(실 라이브 데모=사용자 opt-in 이월). 회고 phase-27.md. P-CONCURRENT-ISOLATION-001 후보. |
| **28** | **2nd brain 학습 루프 (모든 대화 저장 → 강화/정리 → 내 컨셉 수렴)** | **done** (2026-06-17) | ★ 라이브 테스트 갭("홈 입력=막다른 길") 마감. S1 모든 대화 저장+학습 연결(홈도 plans 흐름 → 어느 경로로 써도 brain 적재, 데모 plans 0→1·pkm 0→2) + S2 반복 강화·불필요 제거(PKM consolidation: 유사=강화·dedup, 모순=감점, 미강화=decay/archive + 추출 품질 정련, **스키마 변경 0**) + S3 나만의 컨셉 수렴(상위 강화 PKM → "내 컨셉" 합성·요약 → /brain 상단 + 생성 우선 주입). 코드 main 머지(`aa2d236`). 이월: 강화/decay 장기 실데이터 검증 + 컨셉 합성 human 채점. (archive) |
| **29** | **에이전트 UX 마감 (랜딩·진입·용어·에이전트 느낌 + 멀티모달·동적 그래프)** | **done** (2026-06-17) | ★ "기능 부재가 아니라 도달 실패" 마감 — 신규 제품기능 0, 경험층. S0 Discovery CTA 가림 버그(AppShell HIDDEN_PREFIXES) + S1 랜딩·4상황버튼 + S2 용어번역(표시만, 스키마 불변) + S3 진행요약 패널 + S4 저장후 Brain CTA + S5 에이전트 느낌. 추가: 4버튼 branding goal 분기 + A 멀티모달(레퍼런스 이미지 분석→생성 반영 + 키워드 PKM, FK 23503 회귀 fix) + B1 /brain 동적 force 그래프(react-flow+d3-force). 코드 main 머지(`aa2d236`). 이월: 무도움 완주 라이브 e2e 사용자 실검증 + 멀티모달 "준비중" 실연결. (archive) |
| 30+ | 고도화 / 배포 | future | 배포 Gate B~G(실 staging — 인프라 user-provided) / 품질 후속(🅒) / Custom RAG, LangGraph |
