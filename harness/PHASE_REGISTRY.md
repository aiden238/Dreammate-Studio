...
| **21** | **/brain 4계층 깊이 + 출처 엣지** | **active** (entry 2026-06-04) | Phase 19 그래프 확장 — domain·series 노드(4계층 가시화, DomainRepo/SeriesRepo 신규) + 브랜드 PKM 출처 엣지(source_plan_id→`sourced_from`). additive/graceful(데이터 없으면 기존 불변). 개인 PKM 출처는 source_plan_id 부재로 이월. S1(backend 집계+repos+스키마)→S2(frontend+CC+close). |
