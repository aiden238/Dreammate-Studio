...
| **27** | **1차 MVP 실사용 마감 (실사용 프로파일 + 네비 + rate_limit + 영속·운영 반영 + 첫-사용자 e2e)** | **active** (started 2026-06-05) | ★ 신규 기능 0 — 이미 빌드된 1차 MVP(Phase 1~26 + HIP-006~010)를 **실사용 가능하게 마감**. GPT 전략 판단 + 갭분석 합치(코드 高/실사용·운영 中/품질 弱) → 2차 기능 아닌 **활성화+마감+검증** 우선. S0(HIP→main 머지 a40eb36 ✅) + S1 실사용 프로파일(B-1, output_mode=director + PKM/시드/plans_repo ON, 코드 default OFF 보존) + S2 AppShell 네비(B-2 잔여) + S3 rate_limit 최소(B-7) + S4 plan 영속 ON 검증+migration 적용 스크립트(B-3·B-4·M-1) + S5 첫-사용자 루프 로컬 e2e+human 채점 핸드오프(B-5). 통과 = "처음 온 사용자가 도움 없이 director 기획안 1개 생성·저장·brain축적·다음반영". baseline pytest 802 + audit_naming 0. |
