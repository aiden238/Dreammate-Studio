...
| 25 | 위저드(브랜딩 세션) ↔ 4계층 자동 연결 | **done** (2026-06-04) | 브랜딩 세션 택1(`/branding/select`)에서 topic→domain, format→series 자동 시드(brand 하위, 멱등 get_or_create·gated `branding_pkm_seed_enabled`·graceful) → 위저드 거치면 /brain 4계층 자동 채움. 응답 additive(domain_id/series_id). ★ **라이브 데모 PASS**(select→domain/series 자동 + 멱등 2회=1). (범용 step 위저드는 domain/series discrete 캡처 안 함 → 브랜딩 세션이 연결점, 위저드 재설계는 별건.) CC-032. hermetic pytest 735→749. 🅑 2차. (archive) |
| 26 | 4계층 video 노드 + 개인 PKM 출처 (기능마감 완결) | **done** (2026-06-04) | ① video: VideoProjectRepo 신규 + /me/videos CRUD(4-hop 소유검증) + 그래프 has_video → User→Brand→Domain→Series→**Video** 전 계층 ② 개인 PKM 출처: migration 0007(pkm_entries +source_plan_id) + 추출 훅 plan_id 기록 + 그래프 개인 PKM sourced_from(_append_source_provenance 공유 헬퍼, Phase 21 재사용). ★ **video 라이브 PASS**(CRUD+has_video+미소유 404), 개인 출처=유닛 6. additive/graceful. CC-033/034. hermetic pytest 749→779. 🅑 완결. (archive) |
| **27** | **1차 MVP 실사용 마감 (실사용 프로파일 + 네비 + rate_limit + 영속·운영 반영 + 첫-사용자 e2e)** | **done** (2026-06-05) | ★ 신규 기능 0 — 이미 빌드된 1차 MVP(Phase 1~26 + HIP-006~010)를 **실사용 가능하게 마감**. GPT 전략 판단 + 갭분석 합치(코드 高/실사용·운영 中/품질 弱) → 2차 기능 아닌 **활성화+마감+검증** 우선. S0(HIP→main 머지 a40eb36 ✅) + S1 실사용 프로파일(B-1, APP_PROFILE=realuse → output_mode=director + PKM/시드/plans_repo ON, 코드 default OFF 보존) + S2 AppShell 네비(B-2 잔여) + S3 rate_limit 최소(B-7) + S4 plan 영속 ON 검증+migration 적용 스크립트(B-3·B-4·M-1) + S5 첫-사용자 루프 로컬 e2e+human 채점 핸드오프(B-5). 통과 = "처음 온 사용자가 도움 없이 director 기획안 1개 생성·저장·brain축적·다음반영". ★ 격리 worktree `phase-27-realuse`(동시 HIP-006/007 작업자 `git add -A` 충돌 회피). 결과: hermetic pytest 802→**813**(기존 byte-identical) + scenario_sim 36/36 + audit 0 + frontend build 14/14. 자동검증 close(실 라이브 데모=사용자 opt-in 이월). 회고 phase-27.md. P-CONCURRENT-ISOLATION-001 후보. + project-2 트랙(2026-06-09~10) 통합(693957f): Gemini RAG 임베딩·agent_io 텔레메트리·측정 A7~A11·BOUNDARIES·platform_evolution. |
| **28** | **2nd brain 학습 루프 (모든 대화 저장+학습 → 강화/정리 → 컨셉 수렴)** | **done** (2026-06-08, 정식 close/archive 미경유) | `/GOAL`("쓸수록 내 브랜드 이해하는 2nd brain") 방향. **S1** 모든 대화 저장+학습 연결(홈 막다른길 제거, plans 0→1·pkm 0→2) + **S2** PKM consolidation(유사 강화 confidence+0.15/dedup/노이즈 필터 + decay ×0.95·prune <0.3, ★ **스키마 변경 0**) + **S3** 나만의 컨셉 수렴(`GET /me/concept` + /brain '내 컨셉' 카드, 개인 PKM 종합 → 컨셉 한 줄·기둥·모순쌍, read-time gated). ★ 라이브 11신호 → warm↔neon 모순 자동검출. gated/additive(OFF byte-identical) 회귀 6. 이월: 컨셉→생성 주입. (main `af345ae`) |
| **29** | **에이전트 UX 마감 (랜딩·4상황버튼·멀티모달·동적 그래프·동일출처 프록시)** | **done** (2026-06-09, 정식 close/archive 미경유) | 랜딩 리디자인/4상황버튼 distinct/용어번역(S0~2) + Discovery 진행요약·Brain CTA·에이전트 느낌(S3~5) + **멀티모달 A**(이미지 분석→생성 반영[A1]+키워드 PKM[A2], ★ 라이브 **FK 23503 회귀 수정**, PKM 0→4) + **/brain 동적 force 그래프 B1**(react-flow+d3-force, 드래그+필터, 라이브 10노드/9엣지) + **동일출처 API 프록시**(`/api/*` rewrites → 원격 터널·LAN·localhost/127 교차출처 로그인 문제 해소). tsc 0/lint 0. 이월: 컨셉→생성 주입 / B2 의미 그래프 타이핑 / 시각 e2e. (main `af345ae`) |
| 30+ | 고도화 / 배포 | future | 배포 Gate B~G(실 staging — 인프라 user-provided) / 품질 후속(🅒 human 실채점) / 컨셉→생성 주입 / Custom RAG·LangGraph |
