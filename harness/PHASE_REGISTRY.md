...
| **23** | **품질 정식화 (실 LLM 전수 eval baseline + human review 정비)** | **active** (entry 2026-06-04) | golden_set 25 케이스 실 LLM(rich planning + 9차원 critic) 전수 평가 → overall/depth/verdict 분포 + P0 pass baseline. + Phase 12 S4 human kit LLM-judge 대조 시트(실채점=사용자 deferred). ★ 운영 코드 0 수정(behavior-preserving). 비용 ~$0.3~0.5. S1(전수 baseline)→S2(human 정비+close). 근거 로드맵 D + Gate D 전제. |
