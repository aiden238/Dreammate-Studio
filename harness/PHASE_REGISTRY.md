...
| **12** | **검증 페이즈 (Validation — MVP 품질·가치 실측)** | **next** (기획 대기) | ★ 사용자 지침 "12=검증". 구조 정확성(완료)이 아닌 **출력 품질·가치** 실측: 실 LLM eval ON + golden_set 실점수 + (선택) human review + staging 실사용. slice 단위. scope 확정 후 entry. |
