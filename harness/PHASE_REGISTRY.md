...
| **11** | **LLM Gateway (A안 — registry/alias + OpenAI·Gemini + cross-validation)** | **done** (2026-06-01) | S1 gateway 골격 + S2 cross_validation·Gemini 튜닝 + S3 orchestrator gated hook + 정식화(entry/ADR-039/CC-010). pytest 381→**435** + behavior-preserving(기존 0 수정) + gated default-off + 라이브 입증(OpenAI 생성 + Gemini 교차검증 consensus) + P-X1 63 + 키 커밋 0. agent 6 유지(cross-validation=Critic 추가 pass) |
| **12~20** | **서비스 안정화 / 배포 / LLM Gateway B안** | next | ★ **B안 3-provider 다양성**(GPT/Claude/Gemini 3-plan, 키 준비됨, cost 재조정 §18.B) + full 라이브 /generate(wizard) 데모 + frontend 손-검증 + 배포 Gate B~G + 4계층 linkage / SSE async worker / prompt A/B / 자동 promotion / M3 새 GAP 3(G9/G10/G11) |
