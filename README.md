...
| 항목 | 내용 |
|---|---|
| **Phase** | 🟡 Phase 7 (RAG Lite) — pending planning (Phase 5.5 ✅ done 2026-05-29) — 사용자 명시: "Phase 5.5 진행 후 페이즈 7 기획 시작" |
| **이전 Phase** | Phase 5.5 — Legacy DB Consolidation + Validation Strengthening + Phase 7 Prep ✅ done (2026-05-29, consolidation mini-phase, 4 Slices + A1~A8 + M1~M2 + P-X1 26연속 + PlanCard 18연속 + component_map 28연속 + ADR-023 (Legacy DB 옵션 A) + ADR-024 (Phase 7 RAG scope evolution) + external × 3 self-strengthen V-form + Brand Memory Phase 9+ confirm + pytest 172/172 (+2 deprecation) + smoke 12/12 + scenario_sim v2 10/10 (P-X2 네 번째) + P-LEGACY-CONSOLIDATION-001 신규 후보 + legacy backward-compat 100%) |
| **하네스 규모** | ~78,500줄 (Phase 5.5 +6 신규 / +10 수정 ~+100 backend db deprecated note + ~+250 docs/decisions ADR-023/024 + ~+150 meta validations 강화 + retrospective + closing_notes) |
| **Skill 구조** | `.claude/skills/` 단일 (20개, v1.3.0 + phase-complete v1.2.0 — 26연속 P-X1 입증 + multi-llm-validation formal 정식 확정 + self-strengthen V-form sub-pattern + Legacy DB 옵션 A) |
...
| **next** | **🟡 Phase 7 (RAG Lite — candidate_knowledge 5단계 MVP) — pending planning** (사용자 명시) | **next** | — |
...
2. harness/PROJECT_STATE.md  ← 현재 작업 위치 확인 (pending_phase_7_planning, Phase 5.5 done)
3. harness/phases/archive/phase-5.5-legacy-db-consolidation/closing_notes.md  ← Phase 5.5 종료 메모 + Phase 7 진입 baseline + 권장 Skill
4. harness/phases/archive/phase-5-db-auth/closing_notes.md  ← Phase 5 종료 메모 (참조)
5. harness/phases/archive/phase-6-output-schema-stabilization/  ← Phase 6 contract 안정화 baseline (참조)
6. harness/phases/archive/phase-4.5-critic-revise-loop/  ← Phase 4.5 backend (revise loop + Rewriter) baseline (참조)
7. harness/phases/archive/phase-4-fastapi-extension/  ← Phase 4 backend + frontend baseline (참조)
8. harness/phases/archive/phase-3-pwa-impl/  ← Phase 3 frontend baseline (참조)
9. harness/phases/archive/phase-2-pwa-design/  ← Phase 2 design spec baseline (참조)
10. harness/apps/web/design_handoff.md  ← Phase 2 핵심 산출물 (변경 가이드)
11. harness/meta/retrospectives/phase-5.5.md  ← Phase 5.5 회고 + 개선 제안 §1~3
12. harness/meta/retrospectives/phase-5.md  ← Phase 5 회고 (참조)
13. harness/meta/patterns.md  ← P-X1-EFFECT-001 26연속, P-LEGACY-CONSOLIDATION-001 신규 후보, P-VALIDATION-FORMAL-001 정식 확정 + self-strengthen V-form
14. harness/docs/decisions/phase_7_rag_scope_evolution.md  ← ADR-024 (Phase 7 진입 시 필독)
15. harness/docs/decisions/phase_5_5_legacy_db_consolidation.md  ← ADR-023 (legacy DB 옵션 A)
```
